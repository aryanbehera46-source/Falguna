...
class TTTHQHandler(BaseHTTPRequestHandler):
    ...
    def _html(self, value):
        ...
        self.send_header("Content-Security-Policy", "default-src 'self'; style-src 'unsafe-inline'; script-src 'unsafe-inline'; connect-src 'self' http://127.0.0.1:8765 http://localhost:8765; frame-ancestors 'none'")
