...
# message, so Stop generation can signal a background streaming call to
# actually abort the underlying local-model connection (see
# OllamaProvider.generate_stream) rather than only discarding the result
# once it eventually finishes on its own. Keyed by message_id (not
...
def _model_choice(value):
    """Falguna Local AI Independence V1: a model selector for Chat/Work/
    Research. Returns None for no-override/""/"auto" so it reaches
    ModelRouter.resolve(None) unchanged -- letting the router apply its
    real local-first -> external policy -- and passes any other non-empty
    string through as-is (a bare Codex model id, for full backward
    compatibility, or an explicit 'provider/model' selector such as
    'ollama/qwen2.5:1.5b-instruct'). Previously this substituted the
    Codex default model (MODEL) whenever there was no explicit override,
    which meant every 'Auto' chat silently requested Codex by name and
    never reached the router's local-first selection at all -- a real
    bug that made 'no override' behave like 'always Codex' instead of
    'let the router decide'. Validity of whatever selector comes out of
    here is still checked for real at generate time (ModelRouter.resolve
    raises a clean, sanitized FalgunaModelError/ChatError on anything
    unroutable) -- this function only decides what gets *asked for*."""
...
class FalgunaHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/config":
            ...
            try:
                registry_settings = ModelRegistry(store).load()
                privacy_mode = registry_settings["privacy_mode"]
                # Local AI Independence V1: a real, live, multi-provider
                # model list for the Chat picker (previously it only ever
                # offered `available_models` below, a hardcoded Codex-only
                # list -- a real local model could never even be selected
                # from Chat). Built the same way Settings -> Models does
                # (provider_status_snapshot over build_providers), so it is
                # never stale/invented: health_check()/list_models() run
                # for real, right now, for every enabled provider.
                provider_models = []
                for p in provider_status_snapshot(build_providers(registry_settings)):
                    for m in p["models"]:
                        provider_models.append({
                            "selector": f"{p['provider_id']}/{m['model_id']}",
                            "display_name": m.get("display_name") or m["model_id"],
                            "provider_display_name": p["display_name"],
                            "is_local": bool(m.get("is_local")),
                            # Local AI Independence V1.1: real capability
                            # metadata (see ModelInfo.is_embedding_only) so
                            # the Chat model picker can filter out a model
                            # that cannot answer a generation request at
                            # all (e.g. Ollama's nomic-embed-text). The
                            # backend still independently rejects an
                            # embedding-only selector in
                            # _set_conversation_model/ModelRouter.resolve --
                            # this flag is a frontend convenience, not the
                            # only place this is enforced.
                            "is_embedding_only": bool(m.get("is_embedding_only")),
                            "supports_streaming": bool(m.get("supports_streaming")),
                        })
            finally:
                store.close()
            return self._json({
                ...
                # `available_models` is kept, unchanged, for anything old
                # still reading it -- `provider_models` above is the real,
                # live, local-model-aware list the Chat picker now uses.
        ...
        if path == "/api/usage":
            return self._usage()
        if path == "/api/local-inference/status":
            # Local AI Independence V1.1 Pass D: real-time running/queued
            # counts for the shared local-inference scheduler -- read
            # directly off the live process-global gate, never a stored
            # or stale value (see falguna/scheduler.py).
            return self._json(get_scheduler().snapshot())
        if path == "/api/files":
            return self._files()
    ...
    def _spawn_chat_reply(self, conversation_id, conversation, pending, history):
        ...
        token = secrets.token_urlsafe(16)
        # Local AI Independence V1.1: a real cancellation handle for this
        # specific message, wired all the way down to the underlying
        # provider connection (see OllamaProvider.generate_stream) so Stop
        # generation can actually interrupt local-model inference in
        # progress, not just discard whatever result eventually arrives.
    ...
    def _stop_message(self, conversation_id, message_id):
        ...
        try:
            ...
            # Local AI Independence V1.1: signal the real cancellation
            # event for this message, if a background stream is still
            # running one -- this is what makes Stop close the actual
            # in-flight connection to the local model provider instead of
            # only marking the row CANCELLED and letting inference run to
            # completion unseen. A message that was never streaming (no
            # entry registered, or it already finished) is unaffected.
            with _cancel_events_lock:
                event = _cancel_events.get(message_id)
    ...
    def _set_conversation_model(self, conversation_id, body):
        # Local AI Independence V1: this used to reject anything that
        # wasn't a literal Codex model id, which made it impossible to
        # ever pin a conversation to a local ('ollama/<model>') or
        # custom-endpoint selector even though ModelRouter has supported
        # 'provider/model' selectors since V2.1. Any non-empty string is
        # now accepted here; an actually-unroutable selector still fails
        # cleanly, as a normal ChatError, the first time a reply is
        # generated (ModelRouter.resolve), not as a 400 on save.
        raw = body.get("model")
        if raw is not None and not isinstance(raw, str):
            raise ValueError("Unsupported model")
        model = _model_choice(raw)
        control, store = open_control_plane(self.app_root)
        try:
            chat = ConversationStore(store)
            if not chat.get_conversation(conversation_id):
                return self._json({"error": "conversation not found"}, HTTPStatus.NOT_FOUND)
            if model:
                # Local AI Independence V1.1: reject an embedding-only
                # selector (e.g. a forged 'ollama/nomic-embed-text' request
                # bypassing the Chat picker's own filtering) here too, with
                # the same sanitized message ModelRouter.resolve() would
                # otherwise only raise later, mid-generation. Checked
                # against the provider's real, live capability metadata --
                # a selector referring to an unknown/unreachable provider or
                # model is never blocked here (no false positives); it
                # still fails cleanly at generate time if truly unroutable.
                provider_id, model_id = parse_selector(model)
                if provider_id and model_id:
                    registry_settings = ModelRegistry(store).load()
                    for provider in build_providers(registry_settings):
                        if provider.provider_id != provider_id:
                            continue
                        if ModelRouter._embedding_only_model(provider, model_id):
                            raise ValueError(
                                f"'{model_id}' is an embedding-only model -- it powers local semantic search, "
                                "not chat, research, or work generation. Choose a chat-capable model instead."
                            )
                        break
...
def _run_chat_reply(app_root, token, conversation_id, message_id, history, model, work_mode, project_id=None, cancel_event=None):
    ...
    Local AI Independence V1.1: the reply is now generated through
    ChatResponder.reply_stream rather than reply() -- real, incremental
    text is persisted to the message row (ConversationStore.
    update_streaming_content) as it actually arrives from the underlying
    model, throttled to at most ~1 write per 80ms so a fast local model
    can't write-storm SQLite, with the operation's own state moving to
    STREAMING the moment the first real token lands (never before). The
    final chat.complete_message call below is unconditionally the source
    of truth for the persisted reply regardless of streaming -- it always
    writes the complete, fully-parsed text, so a throttled/missed
    intermediate write is never user-visible in the final result."""
    control, store = open_control_plane(app_root)
    settings = WORK_MODE_SETTINGS.get(work_mode, WORK_MODE_SETTINGS[DEFAULT_WORK_MODE])
    chat = ConversationStore(store)
    try:
        # Local AI Independence V1.1 Pass D: the operation is created as
        # "QUEUED" by _spawn_chat_reply and stays QUEUED -- honestly --
        # until ModelRouter.stream's scheduler actually grants this
        # request a local-inference slot (_on_queue_acquired below), not
        # merely until this background thread happened to start running.
        # A machine with spare capacity acquires near-instantly, so this
        # is invisible in the common case; under real contention it is
        # the difference between an honest queue position and a "THINKING"
        # status that silently wasn't true yet.
        ...
        def _on_queue_acquired() -> None:
            # Fires synchronously, on this background thread, the instant
            # a local-inference slot is actually granted (or immediately,
            # for an external provider -- see ModelRouter.stream). Never
            # regresses a state this operation has already moved past
            # (e.g. if a delta somehow raced ahead onto another thread).
            with _operations_lock:
                if _operations.get(token, {}).get("state") == "QUEUED":
                    _operations[token]["state"] = "THINKING"

        def _on_reply_delta(text_delta: str) -> None:
            partial_chunks.append(text_delta)
            with _operations_lock:
                if _operations.get(token, {}).get("state") in ("QUEUED", "THINKING"):
                    _operations[token]["state"] = "STREAMING"
            now = time.monotonic()
            if now - last_write_at[0] >= 0.08:
                chat.update_streaming_content(message_id, "".join(partial_chunks))
                last_write_at[0] = now

        outcome = ChatResponder(gateway, transport, model, timeout_seconds=settings["chat_timeout"]).reply_stream(
            history, _on_reply_delta, cancel_event=cancel_event,
            memory_context=memory_context["text"] if memory_context else None,
            on_queue_acquired=_on_queue_acquired,
        )
        ...
        applied = chat.fail_message(message_id, safe_message, category=ErrorCategory.UNEXPECTED_FAILURE, detail=str(exc))
        with _operations_lock:
            _operations[token] = {**_operations.get(token, {}), "state": "FAILED" if applied else "CANCELLED", "error": safe_message, "message_id": message_id}
    finally:
        store.close()
        with _cancel_events_lock:
...
async function renderChatToolbar(id,conversation){
  ...
  if(!bar)return;
  let cfg;
  try{cfg=await api('/api/config')}catch(err){cfg={available_models:[],provider_models:[],work_modes:['FAST','BALANCED','DEEP'],default_work_mode:'BALANCED'}}
  // Local AI Independence V1: provider_models is the live, multi-provider
  // list (each entry a real 'provider/model' selector, e.g.
  // 'ollama/qwen2.5:1.5b-instruct'), grouped so Local models are never
  // confused with an External one that leaves this machine. Falls back to
  // the old flat Codex-only list only if provider_models is unexpectedly
  // absent (e.g. mid-rollout against a cached /api/config response).
  // Local AI Independence V1.1: an embedding-only model (e.g. Ollama's
  // nomic-embed-text) can never answer a Chat generation request, so it
  // is filtered out here even though /api/config still reports it (for
  // Settings -> Models). The backend independently rejects an
  // embedding-only selector too (_set_conversation_model / ModelRouter),
  // so this filter is a UX convenience, never the only enforcement.
  const pm=(cfg.provider_models||[]).filter(m=>!m.is_embedding_only);
  const modelOptions=pm.length
    ? ['local','external'].map(kind=>{
        const rows=pm.filter(m=>(kind==='local')===!!m.is_local);
        if(!rows.length)return'';
        return `<optgroup label="${kind==='local'?'Local (runs on this machine)':'External (leaves this machine)'}">${rows.map(m=>`<option value="${esc(m.selector)}" ${conversation.model_override===m.selector?'selected':''}>${esc(m.display_name)} &middot; ${esc(m.provider_display_name)}</option>`).join('')}</optgroup>`;
      }).join('')
...
function pollConversationUntilSettled(id){
  ...
  const tick=async()=>{
    ...
    const last=data.messages[data.messages.length-1];
    if(last&&last.role==='assistant'&&['PENDING','GENERATING'].includes(last.status)){
      // Local AI Independence V1.1: once real generation is under way
      // (GENERATING), poll fast enough that real, incrementally-persisted
      // model output (ConversationStore.update_streaming_content) reads
      // as a smooth, live stream rather than a slow reveal -- still real
      // network responses carrying strictly more real text each time,
      // never a client-side simulated typewriter. While merely QUEUED
      // (PENDING, worker not started yet) the slower interval is enough.
      setTimeout(tick,last.status==='GENERATING'?250:900);
...
function renderMessageRow(m,attachments,isLast){
  ...
  if(status==='PENDING'||status==='GENERATING'){
    // Local AI Independence V1.1: once real streamed text has actually
    // arrived (m.content is non-empty while still GENERATING -- written
    // by ConversationStore.update_streaming_content as genuine
    // incremental model output lands), render it directly instead of the
    // "thinking" placeholder -- a real, growing answer, not a fabricated
    // progress indicator. Before the first token arrives (content still
    // empty) the honest "queued"/"thinking" state is shown exactly as
    // before this phase.
    if(status==='GENERATING'&&m.content){
      return `<div class="msg assistant"><div class="avatar">F</div><div class="bubble">${nl2br(m.content)}</div>
        <div class="status-note">Generating&hellip; <span data-since="${esc(m.created_at)}">0s</span>
...
async function renderSettingsView(tab){
  ...
  if(tab==='appearance'){
    ...
    const providerCard=p=>`<div class="provider-card">
        ...
        ${p.models.length?`<div class="provider-models">${p.models.slice(0,8).map(m=>esc(m.display_name||m.model_id)+(m.is_embedding_only?' <span class="muted">(embedding only)</span>':'')).join(', ')}${p.models.length>8?` +${p.models.length-8} more`:''}</div>`:'<div class="provider-models muted">No model available yet</div>'}
      </div>`;
    body.innerHTML=`<div class="settings-section">
