...
class FalgunaHandler(BaseHTTPRequestHandler):
    server_version = "FalgunaLocal/0.6"
    ...
    def _start(self, body):
        ...
        editable = validate_editable(body.get("editable", []))
        verification_name = body.get("verification") or profile["default_verification"]
        tests = profile["verification_profiles"].get(verification_name)
        if not tests:
            raise ValueError("Select an approved verification profile")
        ...
        thread = threading.Thread(target=_run_mission, args=(self.app_root, token, profile, objective, editable, tests, cap), daemon=True)
        thread.start()
...
def _run_mission(app_root, token, profile, objective, editable, tests, cap):
    control, store = open_control_plane(app_root)
    try:
        commands = [CommandSpec(shlex.split(value), label=f"native-{index}") for index, value in enumerate(tests, 1)]
        policy = RunPolicy(allowed_write_globs=editable, verification_commands=commands, max_cost_usd=cap)
        ...
        transport = CodexCliJSONTransport(Path(codex), codex_home)
...
def serve(root: Path, host="127.0.0.1", port=8765):
    if host not in {"127.0.0.1", "localhost"}:
        raise ValueError("Falguna v0.6 is local-only")
...
<div class="grid"><section class="card"><h2>New Task</h2><form id="mission"><label>Approved project</label><select id="project" required></select><label>Bounded engineering objective</label><textarea id="objective" required placeholder="Describe one small, testable change."></textarea><label>Editable files (one relative path or glob per line)</label><textarea id="editable" required placeholder="falguna/example.py"></textarea><label>Verification profile</label><select id="verification"></select><label>Hard cost cap (USD)</label><input id="cost" type="number" min="0" step="0.01"><div class="muted" id="risk"></div><button id="run" type="submit" style="margin-top:18px">Run Mission</button></form></section>
...
<section class="card wide hidden" id="evidence"><h2>Final Evidence</h2><div class="facts" id="facts"></div><label>Files changed</label><div id="files"></div><label>Unresolved issues</label><div id="issues"></div><div class="actions" id="decisions"><button data-action="approve">Approve Merge</button><button class="danger" data-action="reject">Reject</button><button class="secondary" data-action="request-changes">Request Changes</button></div><p class="muted">A decision records human intent only. This app cannot merge or deploy.</p></section></div></main>
<script>
...
async function boot(){const c=await api('/api/config');c.profiles.forEach(p=>profiles[p.id]=p);$('project').innerHTML=c.profiles.map(p=>`<option value="${p.id}">${p.name}</option>`).join('');selectProject()}
function selectProject(){const p=profiles[$('project').value];if(!p)return;$('verification').innerHTML=Object.keys(p.verification_profiles).map(v=>`<option value="${v}">${v}</option>`).join('');$('verification').value=p.default_verification;$('cost').value=p.default_budget_usd;$('cost').max=p.default_budget_usd;$('risk').textContent='Risk: '+p.risk}
$('project').addEventListener('change',selectProject);$('mission').addEventListener('submit',async e=>{e.preventDefault();$('run').disabled=true;$('empty').classList.add('hidden');$('status').classList.remove('hidden');$('state').textContent='Starting…';$('evidence').classList.add('hidden');try{const out=await api('/api/runs',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({project:$('project').value,objective:$('objective').value,editable:$('editable').value.split('\n').filter(Boolean),verification:$('verification').value,max_cost_usd:Number($('cost').value)})});watchOperation(out.operation)}catch(err){showError(err.message);$('run').disabled=false}});
