...
:root{color-scheme:dark;--bg:#080a0e;--side:#0d1016;--panel:#121720;--soft:#171d27;--line:#252d3a;--text:#f1f4f8;--muted:#929eae;--accent:#79e2b7;--blue:#7db5ff;--warn:#ffc66d;--bad:#ff8c96}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:14px/1.5 Inter,ui-sans-serif,system-ui,-apple-system,sans-serif}button,input,select,textarea{font:inherit}.app{height:100vh;display:grid;grid-template-columns:272px minmax(0,1fr)}aside{background:var(--side);border-right:1px solid var(--line);padding:18px 12px;display:flex;flex-direction:column;min-height:0}.brand{display:flex;align-items:center;gap:10px;padding:4px 8px 18px;font-weight:750;font-size:17px}.mark{display:grid;place-items:center;width:29px;height:29px;border-radius:9px;background:var(--accent);color:#062018;font-weight:900}.new{width:100%;border:1px solid var(--line);background:var(--soft);color:var(--text);border-radius:10px;padding:10px 12px;text-align:left;cursor:pointer}.side-title{padding:22px 8px 7px;color:var(--muted);font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.08em}.projects{display:grid;gap:3px}.project{padding:8px;border-radius:8px;color:#c9d1dc}.history{overflow:auto;display:grid;gap:4px}.history button{border:0;background:transparent;color:var(--text);padding:9px 8px;border-radius:8px;text-align:left;cursor:pointer;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.history button:hover,.history button.active{background:var(--soft)}.history small{display:block;color:var(--muted)}.boundary{margin-top:auto;border-top:1px solid var(--line);padding:14px 8px 2px;color:var(--muted);font-size:12px}.workspace{min-width:0;display:grid;grid-template-rows:auto 1fr auto}.topbar{height:58px;border-bottom:1px solid var(--line);display:flex;align-items:center;justify-content:space-between;padding:0 22px}.topbar strong{font-size:15px}.badge{border:1px solid #285845;color:var(--accent);border-radius:999px;padding:4px 9px;font-size:11px}.conversation{overflow:auto;padding:34px max(22px,calc((100vw - 272px - 850px)/2));}.welcome{max-width:720px;margin:11vh auto 0;text-align:center}.welcome h1{font-size:30px;margin:0 0 9px}.welcome p{color:var(--muted);font-size:16px}.thread{max-width:850px;margin:auto;display:grid;gap:18px}.bubble{border:1px solid var(--line);background:var(--panel);border-radius:16px;padding:18px}.bubble.user{margin-left:12%;background:#151b24}.eyebrow{color:var(--muted);font-size:11px;text-transform:uppercase;letter-spacing:.08em;font-weight:700}.status-line{font-size:21px;font-weight:700;margin:4px 0}.error{color:var(--bad)}.timeline{display:flex;gap:7px;flex-wrap:wrap;margin-top:13px}.step{border:1px solid var(--line);border-radius:999px;padding:5px 9px;color:#cbd4df}.step:last-child{border-color:#326a55;color:var(--accent)}.cards{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;margin-top:14px}.card{background:var(--soft);border:1px solid var(--line);border-radius:11px;padding:12px;min-width:0}.card span{display:block;color:var(--muted);font-size:11px;margin-bottom:3px}.card b{overflow-wrap:anywhere}.details{margin-top:13px;display:grid;gap:10px}.details div{padding:11px 12px;border:1px solid var(--line);border-radius:10px}.details span{color:var(--muted);display:block;font-size:11px}.actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:15px}button.action{border:0;border-radius:9px;padding:9px 13px;font-weight:700;cursor:pointer;background:var(--accent);color:#062018}button.action.secondary{background:#293342;color:var(--text)}button.action.danger{background:#542c34;color:#ffe0e4}button:disabled{opacity:.5;cursor:not-allowed}.composer-wrap{padding:14px max(18px,calc((100vw - 272px - 850px)/2)) 22px;background:linear-gradient(transparent,var(--bg) 18%)}.composer{max-width:850px;margin:auto;background:var(--panel);border:1px solid var(--line);border-radius:16px;padding:11px;box-shadow:0 15px 45px #0008}.composer textarea{display:block;width:100%;min-height:60px;max-height:180px;resize:vertical;border:0;outline:0;background:transparent;color:var(--text);padding:7px}.compose-row{display:flex;align-items:center;gap:9px}.compose-row select,.compose-row input{background:var(--soft);border:1px solid var(--line);color:var(--text);border-radius:8px;padding:7px 9px;min-width:0}.compose-row select{flex:1}.compose-row input{width:88px}.send{border:0;background:var(--accent);color:#062018;border-radius:9px;padding:8px 13px;font-weight:750;cursor:pointer}.hint{max-width:850px;margin:6px auto 0;text-align:center;color:var(--muted);font-size:11px}.mobile-menu{display:none}.hidden{display:none!important}@media(max-width:850px){.app{grid-template-columns:1fr}aside{display:none}.conversation,.composer-wrap{padding-left:15px;padding-right:15px}.cards{grid-template-columns:1fr 1fr}.mobile-menu{display:inline}.topbar{padding:0 15px}}@media(max-width:520px){.cards{grid-template-columns:1fr}.compose-row{flex-wrap:wrap}.compose-row select{flex-basis:100%}.bubble.user{margin-left:0}.conversation{padding-top:20px}.welcome{margin-top:5vh}}
</style></head><body><div class="app"><aside><div class="brand"><span class="mark">F</span>Falguna</div><button class="new" id="newMission">＋ New mission</button><div class="side-title">Approved projects</div><div class="projects" id="projects"></div><div class="side-title">Recent missions</div><div class="history" id="history"></div><div class="boundary">Localhost-only internal alpha<br>No automatic merge or deploy</div></aside><main class="workspace"><header class="topbar"><strong><span class="mobile-menu">F · </span>Engineering work mode</strong><span class="badge">WORK-READY v1</span></header><section class="conversation" id="conversation"><div class="welcome" id="welcome"><h1>What should we build?</h1><p>Choose an approved project and describe the outcome. Falguna discovers the safe file scope and native verification.</p></div><div class="thread hidden" id="thread"><article class="bubble user"><div class="eyebrow">You</div><div id="objectiveText"></div></article><article class="bubble"><div class="eyebrow">Falguna · Work mode</div><div class="status-line" id="state">Preparing mission…</div><div id="failure" class="error"></div><div class="timeline" id="timeline"></div><div class="cards hidden" id="cards"></div><div class="details hidden" id="details"></div><div class="actions hidden" id="actions"><button class="action secondary" id="resume">Resume</button><button class="action" data-action="approve">Approve</button><button class="action danger" data-action="reject">Reject</button><button class="action secondary" data-action="request-changes">Request Changes</button></div></article></div></section><footer class="composer-wrap"><form class="composer" id="mission"><textarea id="objective" required placeholder="Describe a bounded client-work requirement…"></textarea><div class="compose-row"><select id="project" required></select><input id="cost" type="number" min="0" step="0.01" aria-label="Cost cap"><button class="send" id="run" type="submit">Run mission</button></div></form><div class="hint">Human approval stays required. Falguna cannot merge, deploy, or access production secrets.</div></footer></main></div>
<script>
...
async function boot(){const [c,recent]=await Promise.all([api('/api/config'),api('/api/runs')]);c.profiles.forEach(p=>profiles[p.id]=p);$('project').innerHTML=c.profiles.map(p=>`<option value="${esc(p.id)}">${esc(p.name)}</option>`).join('');$('projects').innerHTML=c.profiles.map(p=>`<div class="project">▣ ${esc(p.name)}</div>`).join('');renderHistory(recent.runs);selectProject();const saved=new URLSearchParams(location.search).get('run');if(saved)await openRun(saved)}
function renderHistory(runs){$('history').innerHTML=runs.map(r=>`<button data-run="${esc(r.run_id)}" title="${esc(r.title)}">${esc(r.title)}<small>${esc(r.status.replaceAll('_',' '))}</small></button>`).join('');document.querySelectorAll('[data-run]').forEach(b=>b.onclick=()=>openRun(b.dataset.run))}
function selectProject(){const p=profiles[$('project').value];if(!p)return;$('cost').value=p.default_budget_usd;$('cost').max=p.default_budget_usd}
...
$('project').addEventListener('change',selectProject);$('newMission').onclick=()=>{runId=null;history.replaceState(null,'','/');$('thread').classList.add('hidden');$('welcome').classList.remove('hidden');$('objective').focus()};
